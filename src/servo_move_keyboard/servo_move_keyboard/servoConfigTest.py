...
from i2c_interfaces.msg import Servo, ServoArray, ServoConfig, ServoConfigArray
from i2c_interfaces.srv import ServosConfig
...
class SpotMicroServoControl(Node):
    def __init__(self):
        ...
        self.get_logger().info("Spot Micro Servo Control Node Started")

        # Create a ServoConfig message for one servo
        ...
        print("test1")
        self.config_servos = self.create_client(ServosConfig, "config_servos")
        while not self.config_servos.wait_for_service(timeout_sec=1.0):
            self.get_logger().info(
                "> config_servos service not available, waiting again..."
            )
        self.get_logger().info("> Config_servos service found!")
        print("test2")
        self.resp = self.config_servos.call_async(self._servo_config_msg.servos)
        print("Config servos done!!, returned value: %i" % self.resp.error)
        ...
        self.ros_pub_servo_array = self.create_publisher(
            ServoArray, "/servos_proportional", 10
        )

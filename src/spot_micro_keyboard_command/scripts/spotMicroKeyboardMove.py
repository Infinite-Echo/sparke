...
class SpotMicroKeyboardControl(Node):
    def __init__(self):
        ...
        self._walk_event_cmd_msg.data = (
            True  # Mostly acts as an event driven action on receipt of a true message
        )
        ...
        self._ros_pub_vel_cmd = self.create_publisher(Twist, "/cmd_vel", 10)
        ...
        self.get_logger().info(
            "Keyboard control node publishers corrrectly initialized"
        )
        ...
        while self.ok():
            ...
            if userInput not in valid_cmds:
                self.get_logger().warn(
                    "Invalid keyboard command entered: %s", userInput
                )
            else:
                if userInput == "quit":
                    ...
                    while 1:
                        print(
                            "Cmd Values: phi: %1.3f deg, theta: %1.3f deg, psi: %1.3f deg "
                            % (
                                self._angle_cmd_msg.x * 180 / pi,
                                self._angle_cmd_msg.y * 180 / pi,
                        ...
                        if userInput == "u":
                            ...
                            self.get_logger().warn(
                                "Invalid keyboard command issued in angle command mode: %s",
                                userInput,
                            )
                        else:
                            if userInput == "w":
                                self._angle_cmd_msg.y = (
                    ...
                    while 1:
                        print(
                            "Cmd Values: x speed: %1.3f m/s, y speed: %1.3f m/s, yaw rate: %1.3f deg/s "
                            % (
                        ...
                        if userInput == "u":
                            ...
                            self._ros_pub_stand_cmd.publish(self._stand_event_cmd_msg)
                            self.get_logger().info(
                                "Stand command issued from keyboard."
                            )
                            break
                            ...
                            self.get_logger().warn(
                                "Invalid keyboard command issued in walk mode: %s",
                                userInput,
                            )
                        else:
                            if userInput == "w":
                                ...
                                self._ros_pub_vel_cmd.publish(self._vel_cmd_msg)
                                self.get_logger().info(
                                    "Command issued to zero all rate commands."
                                )
